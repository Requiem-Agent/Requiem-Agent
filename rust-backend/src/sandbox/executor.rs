...
impl SandboxExecutor {
    ...
    fn build_layers(
        ...
        let rlimit = if language == Language::Rust {
            ...
            RlimitLayer::new()
                .with_memory(COMPILE_MEMORY_BYTES)
                .with_cpu(MAX_COMPILE_TIMEOUT_SECS)
        } else {
            RlimitLayer::new()
                .with_memory(MAX_OUTPUT_SIZE as u64 * 2)
                .with_cpu(MAX_TIMEOUT_SECS)
        };
...
"#
            ...
            fs::write(&cargo, toml).await.map_err(|e| CompileError {
                stderr: format!("Cargo.toml: {e}"),
                exit_code: -1,
                timed_out: false,
            })?;
        }
        ...
        match out {
            ...
            Ok(Ok(o)) => Err(CompileError {
                stderr: String::from_utf8_lossy(&o.stderr).to_string(),
                exit_code: o.status.code().unwrap_or(-1),
                timed_out: false,
            }),
            Ok(Err(e)) => Err(CompileError {
                stderr: format!("cargo: {e}"),
                exit_code: -1,
                timed_out: false,
            }),
            Err(_) => Err(CompileError {
                stderr: "Compile timeout 180s".into(),
                exit_code: -1,
                timed_out: true,
            }),
        }
    ...
    async fn compile_go(dir: &Path, filename: &str) -> Result<(), CompileError> {
        ...
        match out {
            ...
            Err(_) => Err(CompileError {
                stderr: "Go compile timeout".into(),
                exit_code: -1,
                timed_out: true,
            }),
        }
    ...
    async fn run(
        ...
        if let Err(e) = layers.apply_child_all() {
            return SandboxResult::err(
                language,
                format!("Sandbox layers (child): {}", e.join("; ")),
            );
        }
        ...
        let mut env = vec![
            ...
            (
                "PATH".into(),
                "/usr/local/bin:/usr/bin:/bin:/usr/local/cargo/bin:/root/.bun/bin".into(),
            ),
        ...
        match language {
            Language::Rust => {
                let bin = dir
                    .join("target")
                    .join("debug")
                    .join(filename.trim_end_matches(".rs"));
                if bin.exists() {
                    Self::exec(&bin, &[], dir, &env, timeout_secs, language).await
                } else {
                    SandboxResult::err(language, "Binary not found after compile".into())
                }
            }
            Language::Python => {
                Self::exec(
                    &PathBuf::from("python3"),
                    &[dir.join(filename).to_str().unwrap()],
                    dir,
                    &env,
                    timeout_secs,
                    language,
            ...
            Language::TypeScript => {
                Self::exec(
                    &PathBuf::from("bun"),
                    &["run", dir.join(filename).to_str().unwrap()],
                    dir,
                    &env,
                    timeout_secs,
                    language,
    ...
    pub async fn cleanup(user_id: &str, session_id: &str) -> Result<(), String> {
        ...
        if d.exists() {
            fs::remove_dir_all(&d).await.map_err(|e| format!("{e}"))?;
        }
        Ok(())
    }
            ...
            if let Ok(e) = std::fs::read_dir(&p) {
                for entry in e.flatten() {
                    ...
                    if path.is_dir() {
                        ...
                        *t += std::fs::metadata(&path).map(|m| m.len()).unwrap_or(0);
                    }
                }
            }
        }
...
mod tests {
    ...
    async fn test_execute_bash_echo() {
        ...
        // Cleanup
        SandboxExecutor::cleanup("test_user", "test_session")
            .await
            .ok();
    }

...
async fn main() -> Result<()> {
    ...
    let turso_url = match std::env::var("TURSO_URL") {
        Ok(u) => {
            eprintln!("[requiem] TURSO_URL set: {}", &u[..u.len().min(30)]);
            u
        }
        Err(e) => {
            eprintln!("[requiem] TURSO_URL missing: {e}");
            std::process::exit(1);
        }
    };
    let turso_token = std::env::var("TURSO_AUTH_TOKEN").ok();
    eprintln!(
        "[requiem] TURSO_AUTH_TOKEN present: {}",
        turso_token.is_some()
    );
    ...
    let state = match AppState::new(&turso_url, turso_token).await {
        Ok(s) => {
            eprintln!("[requiem] Turso connected");
            s
        }
        Err(e) => {
            eprintln!("[requiem] Turso connect FAILED: {e:#}");
            std::process::exit(1);
        }
    };
    ...
    // تهيئة محرك الوكيل
    let agent_engine = Arc::new(RwLock::new(crate::agent::AgentEngine::new(
        "system",
        crate::agent::protocol::mode::AgentMode::Autonomous,
        audit_log.clone(),
        state.conn.clone(),
    )));
    ...
    // تهيئة حالة المهام
    let task_state: routes::tasks::SharedTaskState =
        Arc::new(RwLock::new(routes::tasks::TaskState::new()));
    ...
    let context_router: routes::anti_printer::SharedRouter =
        Arc::new(RwLock::new(crate::agent::anti_printer::ContextRouter::new()));
    ...
    let protected_router = Router::new()
        ...
        .route(
            "/sessions/{id}/messages",
            post(routes::messages::add_message),
        )
        ...
        .route(
            "/files/{name}",
            get(routes::files::get_user_file).delete(routes::files::delete_user_file),
        )
        ...
        .route(
            ...
            post(routes::formats::validate_format),
        )
        .route(
            "/formats/{name}/format",
            post(routes::formats::format_content),
        )
        .route(
            "/formats/{name}/convert",
            post(routes::formats::convert_format),
        )
        .route(
            "/formats/markdown/preview",
            post(routes::formats::preview_markdown),
        )
        ...
        .route(
            "/enforce/audit/user/{user_id}",
            get(routes::enforce::get_audit_by_user),
        )
        .route(
            "/enforce/audit/recent/{n}",
            get(routes::enforce::get_recent_audit),
        )
        .route(
            "/enforce/audit/stats",
            get(routes::enforce::get_audit_stats),
        )
        .route(
            "/enforce/check-security",
            post(routes::enforce::check_security),
        )
        .route(
            "/enforce/validate-path",
            post(routes::enforce::enforce_validate_path),
        )
        ...
        .route(
            "/agent/mode/current",
            get(routes::agent_protocol::current_mode),
        )
        .route(
            "/agent/mode/auto-switch",
            post(routes::agent_protocol::auto_switch_mode),
        )
        .route(
            "/agent/mode/history",
            get(routes::agent_protocol::mode_history),
        )
        .route(
            "/agent/protocol/thinking/validate",
            post(routes::agent_protocol::validate_thinking),
        )
        .route(
            "/agent/compiler/correct",
            post(routes::agent_protocol::correct_json),
        )
        .route(
            "/agent/compiler/compile",
            post(routes::agent_protocol::compile_output),
        )
        .route(
            "/agent/sub/spawn",
            post(routes::agent_protocol::spawn_sub_agent),
        )
        .route(
            "/agent/sub/{id}/progress",
            get(routes::agent_protocol::sub_agent_progress),
        )
        .route(
            "/agent/sub/{id}/cancel",
            post(routes::agent_protocol::cancel_sub_agent),
        )
        .route(
            "/agent/sub/merge",
            post(routes::agent_protocol::merge_sub_agents),
        )
        .route(
            "/agent/sub/list",
            get(routes::agent_protocol::list_sub_agents),
        )
        .route(
            "/agent/environment",
            get(routes::agent_env::get_environment),
        )
        .route("/agent/skills", get(routes::agent_env::list_skills))
        .route(
            "/agent/skills/suggest",
            post(routes::agent_env::suggest_skills),
        )
        .route(
            "/agent/skills/execute",
            post(routes::agent_env::execute_skill),
        )
        // ─── Tasks Routes ─────────────────────────────────────────────
        ...
        .route(
            "/user/question/{id}",
            get(routes::user_questions::get_question),
        )
        .route(
            "/user/question/{id}/answer",
            put(routes::user_questions::answer_question),
        )
        .route(
            "/user/question/pending",
            get(routes::user_questions::pending_questions),
        )
        .route(
            "/user/question/{id}/cancel",
            post(routes::user_questions::cancel_question),
        )
        .route(
            "/user/question/stats",
            get(routes::user_questions::question_stats),
        )
        // ─── Phase 15 — Anti-Printer Routes ───────────────────────────
        .route(
            "/agent/semantic/analyze",
            post(routes::anti_printer::semantic_analyze),
        )
        .route(
            "/agent/semantic/context",
            get(routes::anti_printer::semantic_context),
        )
        .route(
            "/agent/anti-printer/check",
            post(routes::anti_printer::anti_printer_check),
        )
        .route(
            "/agent/anti-printer/pipeline",
            post(routes::anti_printer::anti_printer_pipeline),
        )
        .route(
            "/agent/router/route",
            post(routes::anti_printer::router_route),
        )
        .route(
            "/agent/router/models",
            get(routes::anti_printer::router_models),
        )
        .route(
            "/agent/router/strategy",
            post(routes::anti_printer::router_strategy),
        )
        .route(
            "/agent/router/outcome",
            post(routes::anti_printer::router_outcome),
        )
        // ─── Phase 16 — Synergy Routes ────────────────────────────────
        .route("/agent/synergy/run", post(routes::synergy::synergy_run))
        .route(
            "/agent/synergy/pattern",
            put(routes::synergy::synergy_set_pattern),
        )
        .route(
            "/agent/synergy/history",
            get(routes::synergy::synergy_history),
        )
        .route(
            "/agent/synergy/report",
            get(routes::synergy::synergy_report),
        )
        .route(
            "/agent/synergy/performance",
            get(routes::synergy::synergy_performance),
        )
        ...
        .route(
            "/rag/memory/{id}",
            get(routes::rag::get_memory).delete(routes::rag::delete_memory),
        )
        ...
        .route(
            "/locks/context",
            post(routes::strict_locks::get_lock_context),
        )
        // ─── Identity Shield v3 Routes ─────────────────────────────────
        .route(
            "/identity/check",
            post(routes::identity_shield::check_identity),
        )
        .route("/identity/stats", get(routes::identity_shield::get_stats))
        .route(
            "/identity/prompt",
            get(routes::identity_shield::get_identity_prompt),
        )
        .route(
            "/identity/cutoff",
            post(routes::identity_shield::check_cutoff),
        )
        .route(
            "/identity/developer",
            get(routes::identity_shield::get_developer_info),
        )
        ...
        .route(
            "/ws/terminal",
            get(routes::ws_terminal::ws_terminal_handler),
        )
        .route(
        ...
        .route(
            "/preferences",
            get(routes::preferences::get_preferences::<AppState>),
        )
        .route(
            "/preferences",
            put(routes::preferences::put_preferences::<AppState>),
        )
        // S6-02: User API keys (encrypted LLM provider keys)
        .route(
            "/user-api-keys",
            get(routes::user_api_keys::list_api_keys::<AppState>),
        )
        .route(
            "/user-api-keys",
            post(routes::user_api_keys::save_api_key::<AppState>),
        )
        .route(
            "/user-api-keys/{id}",
            delete(routes::user_api_keys::delete_api_key::<AppState>),
        )
        .route(
            "/user-api-keys/decrypt",
            post(routes::user_api_keys::decrypt_api_key_handler::<AppState>),
        )
        // ─── Workspace Routes ─────────────────────────────────────────────────
        ...
        .route(
            ...
            get(routes::workspaces::read_file)
                .put(routes::workspaces::write_file)
                .delete(routes::workspaces::delete_file),
        )
        .route(
            "/workspaces/{id}/mkdir/{*path}",
            post(routes::workspaces::mkdir),
        )
        .route(
            "/workspaces/{id}/clone",
            post(routes::workspaces::clone_repo),
        )
    ...
    let listener = match tokio::net::TcpListener::bind(format!("0.0.0.0:{port}")).await {
        Ok(l) => {
            eprintln!("[requiem] listening on port {port}");
            l
        }
        Err(e) => {
            eprintln!("[requiem] bind FAILED: {e}");
            std::process::exit(1);
        }
    };

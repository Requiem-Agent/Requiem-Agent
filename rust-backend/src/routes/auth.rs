...
use crate::{AppState, auth, routes::AuthUserExt};
...
pub async fn login_handler(
    ...
    let plan = match state.popcorn_conn.query(
...
pub async fn me_handler(
    ...
    match rows.next().await.map_err(|e| {
        ...
        None => Err((StatusCode::UNAUTHORIZED, Json(json!({ "error": "User not found" })))),
    }
}

...
pub enum ModelEvent {
    ...
    Slow { latency_ms: u64 },
    Error { error: String },
    Success { latency_ms: u64, tokens: u32 },
}
...
pub fn assign_role(mid: &str, mname: &str) -> &'static str {
    ...
    if l.contains("deepseek")
        || l.contains("code")
        || l.contains("big-pickle")
        || l.contains("bigpickle")
    {
        "coder"
    } else if l.contains("mimo") || l.contains("orchestrat") {
        "orchestrator"
    } else if l.contains("hy3")
        || l.contains("reasoning")
        || l.contains("planner")
        || l.contains("o3")
    {
        "planner"
    } else if l.contains("north") || l.contains("review") || l.contains("audit") {
        "reviewer"
    } else if l.contains("nometron") || l.contains("debug") || l.contains("large") {
        "debugger"
    } else if l.contains("design") || l.contains("vision") {
        "designer"
    } else if l.contains("research") || l.contains("web") || l.contains("search") {
        "researcher"
    } else if l.contains("secur") || l.contains("cyber") {
        "security"
    } else {
        "general"
    }
...
impl ModelRegistry {
    pub fn new() -> Arc<Self> {
        let capabilities = vec![
            ModelCapability {
                ...
                roles: vec!["code", "general", "debug", "explore", "security"],
                vision: false,
                speed: 9,
                reliability: 0.85,
                max_tokens: 32768,
            ...
            ModelCapability {
                id: "big-pickle",
                name: "Big Pickle",
                roles: vec!["multi-file", "code", "review"],
                vision: false,
                speed: 7,
                reliability: 0.80,
                max_tokens: 65536,
            ...
            ModelCapability {
                id: "hy3-free",
                name: "Hy3",
                roles: vec!["plan", "research", "debug", "review", "security"],
                vision: false,
                speed: 5,
                reliability: 0.90,
                max_tokens: 65536,
    ...
    pub fn get_capability(&self, id: &str) -> Option<&ModelCapability> {
        self.capabilities.iter().find(|m| m.id == id)
    }
    ...
    pub async fn select_for_task(&self, category: &str, effort: &str) -> ModelSelection {
        ...
        let cand: Vec<&ModelCapability> = self
            .capabilities
            .iter()
            .filter(|c| c.roles.contains(&category))
            .collect();
        if cand.is_empty() {
            ...
            return ModelSelection {
                ...
                reason: format!("no '{}' category", category),
                fallback_chain: self.capabilities.iter().map(|m| m.id.to_string()).collect(),
            };
        }
        let mut scored: Vec<(&ModelCapability, f32)> = cand
            .iter()
            .filter_map(|c| {
                ...
                let s = if heavy {
                    rel * (c.max_tokens as f32 / 32768.0).min(4.0)
                } else {
                    rel * (c.speed as f32 / 10.0)
                };
                Some((*c, s))
            })
            .collect();
        ...
        if scored.is_empty() {
            let f = self
                ...
                .iter()
                .find(|c| health.get(c.id).map(|h| h.available).unwrap_or(false))
                .unwrap_or(&self.capabilities[0]);
            return ModelSelection {
                model_id: f.id.to_string(),
                reason: "force".into(),
                fallback_chain: self.capabilities.iter().map(|m| m.id.to_string()).collect(),
            };
        }
        ModelSelection {
            model_id: scored[0].0.id.to_string(),
            reason: format!("{:.2}", scored[0].1),
            fallback_chain: scored
                .iter()
                .skip(1)
                .take(3)
    ...
    pub async fn record_event(&self, model_id: &str, event: ModelEvent) {
        ...
        let st = s.entry(model_id.to_string()).or_insert(ModelStats {
            total_calls: 0,
            failed_calls: 0,
            total_latency_ms: 0,
            total_tokens: 0,
        ...
        match event {
            ModelEvent::Success { latency_ms, tokens } => {
                st.total_calls += 1;
                st.total_latency_ms += latency_ms;
                st.total_tokens += tokens as u64;
        ...
        if let Some(h) = health.get_mut(model_id) {
            h.total_calls = st.total_calls;
            h.failed_calls = st.failed_calls;
            h.success_rate = if st.total_calls > 0 {
                (st.total_calls - st.failed_calls) as f32 / st.total_calls as f32
            } else {
                1.0
            };
            ...
            match event {
                ...
                ModelEvent::Offline => {
                    h.available = false;
                    warn!("{} OFFLINE", model_id);
                }
                ModelEvent::Error { .. } => {
                    if h.failed_calls > 3 && h.success_rate < 0.3 {
                        h.available = false;
                        warn!("{} auto-offline", model_id);
                    }
                }
                _ => {}
            }
    ...
    pub async fn probe_all(&self) -> Vec<(String, bool)> {
        ...
        for c in &self.capabilities {
            r.push((c.id.to_string(), self.probe_model(c.id).await));
        }
        self.last_probe.store(
...
pub fn registry() -> Arc<ModelRegistry> {
    MODEL_REGISTRY.get_or_init(|| ModelRegistry::new()).clone()
}
pub fn init_registry() {
    registry();
    info!("Model Registry ready");
}
pub async fn pick_model(category: &str, effort: &str) -> ModelSelection {
    registry().select_for_task(category, effort).await
}
pub async fn probe_all_models() -> Vec<(String, bool)> {
